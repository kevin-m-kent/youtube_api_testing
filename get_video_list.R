...
playlist_id <- test %>%
  pluck("items", 1, "contentDetails", "relatedPlaylists", "uploads")

req_2 <- request(glue::glue("https://www.googleapis.com/youtube/v3/playlistItems?part=snippet%2CcontentDetails&maxResults=100&playlistId={playlist_id}"))
...
all_ids <- test_2 %>%
  pluck("items") %>%
  map(~ pluck(.,"contentDetails",  "videoId"))

all_ids_csv <- all_ids %>%
  paste(collapse = ",")

req_video_details <- request(glue::glue("https://youtube.googleapis.com/youtube/v3/videos?part=processingDetails&part=snippet%2CcontentDetails%2Cstatistics&id={all_ids_csv}"))
...
test <- video_details %>% resp_body_json()
